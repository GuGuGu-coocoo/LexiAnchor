...
词卡搜索使用 SQLite FTS5 `unicode61` tokenizer。schema v3 会回填已有 v2 词卡，并通过
触发器同步新增、编辑、软删除和恢复；列表查询限制为 200 条，备份导出仍读取完整快照。
10 万条自制词卡的精确前缀查询已在 SQLite WASM 内存库中通过交互性基线。
...
- 110,140,568 字节自制 PDF 已通过导入、首屏渲染和翻页；发布前继续扩展真实公版大文件矩阵。
...
词卡可单独导出为 `lexianchor.word-cards` schema v1 JSON 备份。导入器先校验格式、必填内容、
时间戳和版本，再在一个事务中 upsert；设备本地 `sourceBookId` 不跨设备携带，来源书名和原句
继续保留。完整应用导出包仍按 8.4 节的结构逐步实现。

书籍删除先在 SQLite 事务中写入 book tombstone，并按用户选择保留或清除阅读进度、软删除
关联词卡；事务成功后再删除 `ContentStore` 中的内容哈希文件。若文件清理失败，活动数据库
