...
来源书名和原句只能存在一张活动词卡；删除后允许再次收藏。搜索覆盖词语、释义、词根/词源、
书名和原句，并完全在 SQLite Worker 中执行。
...
| Web 离线重载后查看、搜索、编辑、备份恢复和删除词卡 | 通过 |
| 书库搜索/排序、删除确认与可选数据保留 | 通过 |
| 删除 OPFS 书籍副本并保留进度后重新导入恢复 | 通过 |
...
- 已支持批量文件选择、拖放导入和书名/作者/语言编辑；
- 内容写入与数据库记录是两个步骤，异常中断可能留下没有引用的内容文件；后续需要孤儿清理；
- 已实现包含书籍、设置、阅读进度和词卡的完整导出/恢复包、配额显示和由用户主动触发的
  `navigator.storage.persist()` 请求；
- 词卡已实现全部学习字段编辑、版本化 JSON 导出/原子恢复和删除撤销；
- schema v3 使用 FTS5 `unicode61` 索引并从 v2 回填，10 万条自制词卡的前缀搜索通过
  低于 1 秒的自动化门槛；
