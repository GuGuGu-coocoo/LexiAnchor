...
- [x] EPUB 2 NCX 与 EPUB 3 Navigation Document 目录；
- [x] 目录层级显示、当前章节标记和章节跳转；
- [x] 方向键与 Page Up/Page Down 翻页；
- [x] 字体/字重、字号、行距、词间距、字间距、正文宽度、对齐和主题颜色；
- [x] 全局默认值与单本书排版偏好持久化；
- [x] 可关闭的弱/中/强三级焦点加粗，EPUB/PDF 共用产品规则；
- [x] 窗口/全屏；
- [x] 中英法 UI 基础。
...
- [x] 按书名或作者搜索；
- [x] 按最近阅读、导入时间、书名和进度排序；
- [x] 删除前确认，并分别选择是否保留阅读进度和词卡；
- [x] 数据库事务软删除同步记录，随后清理 OPFS 书籍副本；
- [ ] 批量/拖放导入、列表视图和元数据编辑。
